...
five <- head(inc,5)
kbl(five, booktabs=T,caption="Income")  %>%
...
library(kableExtra)
inc <- read.csv("data/income.csv")
ecdf.fun = ecdf(inc$Income)
 

inc = inc %>% mutate(cdf = ecdf.fun(inc$Income)) %>% mutate(zscore = scale(Income))
inc$zscore = round(inc$zscore,digits=1)

five <- head(inc,10)

kbl(five, booktabs=T, caption="Income, CDF, zscore")  %>%
  kable_styling(latex_options = "striped")
